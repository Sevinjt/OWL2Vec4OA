...
def extract_owl2vec_model(ontology_file1, ontology_file2, mapping_file, config_file, uri_doc = True, lit_doc = True, mix_doc = True):
    ...
    if ontology_file1:
        config['BASIC']['ontology_file1'] = click.format_filename(ontology_file1)
    if ontology_file2:
        config['BASIC']['ontology_file2'] = click.format_filename(ontology_file2)
    if mapping_file:
...
def __perform_ontology_embedding(config):
    ...
    if ('ontology_projection' in config['DOCUMENT'] and config['DOCUMENT']['ontology_projection'] == 'yes') or \
        ...
        projection.serialize(config['BASIC']['ontology_file'], format="turtle")      
    ...
    if 'pre_entity_file' in config['DOCUMENT']:
        ...
        #     entities.add(str(a))
        #     entities.add(str(c))
        entities = class_onto1.union(class_onto2)
    ...
    if 'URI_Doc' in config['DOCUMENT'] and config['DOCUMENT']['URI_Doc'] == 'yes':
        ...
        walks_,instances = get_rdf2vec_walks(ontology_file=ontology_file, mapping_file=mapping, walker_type=config['DOCUMENT']['walker'],
                                walk_depth=int(config['DOCUMENT']['walk_depth']), classes=[], config = config)
